...
# Exclude nonemp from classification levels; pi_0() reads these from the caller env
macro_levels = setdiff(macro_order, "nonemp")
meso_levels  = setdiff(meso_order,  "nonemp")

cat(sprintf("Employed sample: %d father-son pairs\n", nrow(data)))
...
desc_region    = weighted_prop_table(data, region) |> mutate(prop = round(prop, 1))
...
verify_ergodic(P_macro_global, "macro global (employed)")
verify_ergodic(P_meso_global,  "meso global (employed)")
...
macro_level_order = macro_order[macro_order != "nonemp"]
meso_level_order  = meso_order[meso_order   != "nonemp"]
...
om_plot = ggplot(om_total, aes(x = t, y = est)) +
  ...
  scale_fill_manual(name = "Level",
                    labels = c("Meso (5 categories)", "Macro (3 categories)"),
...
# nonemp rows and nonemp-specific stats are dropped; p_nonman_fm_farming replaces
# p_nonemp_fm_farming as the third farming-origin transition of interest.
compute_mobility_stats = function(df) {
  ...
  p_nonman_given_farming  = weighted.mean(
    farming_rows$macro_son == "nonmanual", farming_rows$w_atc_norm, na.rm = TRUE)

  # Marginal son distribution
  p_son_nonmanual = weighted.mean(df$macro_son == "nonmanual", df$w_atc_norm, na.rm = TRUE)
  p_son_farming   = weighted.mean(df$macro_son == "farming",   df$w_atc_norm, na.rm = TRUE)
  ...
  tibble(
    ...
    p_nonman_fm_farming  = round(p_nonman_given_farming, 3),
    p_son_nonmanual      = round(p_son_nonmanual, 3),
    p_son_farming        = round(p_son_farming, 3)
  )
}
...
sm_plot = plot_region_map(states_sf, regions_sf, centroids,
                          sm, sm,
                          "Structural Mobility by Region (Employed)")

em_plot = plot_region_map(states_sf, regions_sf, centroids,
                          em, em,
                          "Exchange Mobility by Region (Employed)")

p_manual_plot = plot_region_map(states_sf, regions_sf, centroids,
                                p_manual_fm_farming, p_manual_fm_farming,
                                "P(Manual | Father Farming) by Region (Employed)")

p_farming_plot = plot_region_map(states_sf, regions_sf, centroids,
                                 p_farming_fm_farming, p_farming_fm_farming,
                                 "P(Farming | Father Farming) by Region (Employed)")

p_nonman_plot = plot_region_map(states_sf, regions_sf, centroids,
                                p_nonman_fm_farming, p_nonman_fm_farming,
                                "P(Nonmanual | Father Farming) by Region (Employed)")
...
cat("\n--- Cohort stationarity: OM(1) and d'(1) by birth cohort (employed) ---\n")
print(cohort_tbl)

cat("\n--- Macro transition matrices by cohort (employed) ---\n")
...
# Some rows with occ <= 970 in the main classification may be reclassified to
# nonemp under the attachment-first scheme (occ_pop_alt or occ_son_alt = 999).
# Those are additionally dropped here so both father and son remain employed
# under the alt classification as well.
data_alt = data |>
  ...
  filter(macro_pop != "nonemp", macro_son != "nonemp") |>
  mutate(w_atc_norm = w_atc_norm / sum(w_atc_norm) * n())

cat(sprintf("Alt employed sample: %d father-son pairs\n", nrow(data_alt)))
...
verify_ergodic(P_macro_alt_global, "macro alt global (employed)")
...
comparison_tbl = bind_rows(
  stats_main |> mutate(matrix = "main (occ-first, employed)"),
  stats_alt  |> mutate(matrix = "alt (attachment-first, employed)")
) |> select(matrix, everything())

cat("\n--- Main vs Alt: global mobility statistics (employed) ---\n")
...
) |> select(matrix, region, n, sm, em, p_manual_fm_farming,
            p_farming_fm_farming, p_nonman_fm_farming)

cat("\n--- Main vs Alt: regional mobility statistics (employed) ---\n")
...
cat("\n--- Regional summary table (employed) ---\n")
print(
  knitr::kable(
    regional_table,
    col.names = c("Region", "N", "SM", "EM",
                  "P(Manual|Farm)", "P(Farm|Farm)", "P(Nonmanual|Farm)"),
    digits  = 3,
    caption = "Regional mobility statistics — employed sample (weighted macro 3x3)"
  )
)
...
cat("\n--- SM share of OM at t=1 (macro, employed) ---\n")
print(em_sm_ratio)
...
cat(sprintf("\n--- Age restriction: employed n = %d, 25-44 n = %d (%.0f%% loss) ---\n",
...
robustness_tbl = bind_rows(
  stats_main |> mutate(sample = "main employed (20-44)"),
  stats_2544 |> mutate(sample = "restricted employed (25-44)")
) |> select(sample, everything())

cat("\n--- Robustness: main vs 25-44 restricted sample (employed) ---\n")
print(robustness_tbl)

P_2544 = p_matrix(data_2544, macro_pop, macro_son)
cat("\nRestricted (25-44) macro transition matrix (employed):\n")
...
dir.create("output/figures/employed", recursive = TRUE, showWarnings = FALSE)

ggsave("output/figures/employed/pmat_macro.png",       combined_plot_macro,     width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/pmat_meso.png",        combined_plot_meso,      width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/om_plot.png",          om_plot,                 width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_sm.png",           sm_plot,                 width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_em.png",           em_plot,                 width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_p_manual.png",     p_manual_plot,           width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_p_farming.png",    p_farming_plot,          width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_p_nonman.png",     p_nonman_plot,           width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_sm_alt.png",       sm_plot_alt,             width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_em_alt.png",       em_plot_alt,             width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_p_manual_alt.png", p_manual_plot_alt,       width = 10, height = 6, dpi = 300)
ggsave("output/figures/employed/map_p_farming_alt.png",p_farming_plot_alt,      width = 10, height = 6, dpi = 300)
